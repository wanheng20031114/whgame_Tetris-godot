...
## 10. Godot 验证进程收尾规则（2026-05-30）

进行 Godot 验证时，即使只是执行 `godot --headless --path . --check-only`，也必须把进程管理视为测试流程的一部分。

协作者或自动化助手在启动 Godot 前，应尽量记录本次启动的进程或使用可识别的启动参数；检查结束后，必须确认对应 Godot 进程已经退出，防止残留 `Godot.exe` 长时间占用 CPU、内存或调试端口。

清理进程时必须避免误伤用户手动打开的 Godot 编辑器或游戏实例。优先按本次启动的 PID、命令行参数、工作目录、`--headless`、`--check-only`、调试端口等条件精确定位；不能仅凭进程名粗暴关闭所有 `Godot.exe`。如果无法确认某个 Godot 进程是否由本次任务创建，应先向用户确认。
