const translations = {
  "zh-CN": {
    ...
    "intro_p3": "因此，俄罗斯方块的玩家也可以被看作选手。选手需要练习操作速度，也需要理解攻击效率、堆叠结构、预览判断、Hold 的使用、T-Spin、Combo、Back-to-Back 等概念。这些概念组合起来，构成了一个有学习曲线、有策略深度、也有复盘价值的竞技系统。",
  ...
  "ja": {
    ...
    "intro_p3": "そのため、テトリスプレイヤーは「競技選手（プレイヤー）」とも見なせます。選手は操作スピードを練習するだけでなく、攻撃効率、積み方の構造、NEXT（次に来るブロック）の判断、Hold の使い方、T-Spin、Combo、Back-to-Back などの概念を理解する必要があります。これらの概念が組み合わさることで、学習曲線と戦略的な深みを持ち、リプレイ分析の価値がある競技システムが形成されます。",
  ...
  "en": {
    ...
    "intro_p3": "Therefore, Tetris players can also be viewed as competitors. Competitors need to practice operation speed, and also understand concepts like attack efficiency, stacking structure, preview judgment, the use of Hold, T-Spin, Combo, and Back-to-Back. These concepts combine to form a competitive system with a learning curve, strategic depth, and replay value.",
